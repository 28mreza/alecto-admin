...
/**
 * Preview icon for the "system" theme option.
 * Mirrors `icon-theme-system.tsx` from the React source. Unlike the
 * light/dark icons, the root svg carries the checked-state fill/stroke
 * classes so it tints with the theme color when selected.
 */
@Component({
  ...
  template: `
    <svg
      ...
      class="fill-primary stroke-primary group-data-[state=unchecked]:fill-muted-foreground group-data-[state=unchecked]:stroke-muted-foreground overflow-hidden rounded-[6px]"
      [class]="className()"
      aria-hidden="true"
    >
